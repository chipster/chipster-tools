...
#   # TODO we would probably need this (or without -dev) in comp-24.04-r-deps too
...
# new runtimes mount tools-bin here, and emboss doesn't work if it's moved
# e.g. "emboss-20.04/bin/seqret: error while loading shared libraries: libnucleus.so.6: cannot open shared object file: No such file or directory"
TOOLS_PATH="/opt/chipster/tools-bin"
EMBOSS_PATH=${TOOLS_PATH}/EMBOSS-6.5.7-24.04
...
bash $BUNDLE_SCRIPTS_DIR/run-in-pod.bash $JOB_NAME $BUILD_NUMBER ubuntu - <<EOF
  ...
  ln -s EMBOSS-6.5.7-24.04 ${TOOLS_PATH}/emboss-24.04
...
bash $BUNDLE_SCRIPTS_DIR/move-to-artefacts.bash $TOOLS_PATH/EMBOSS-6.5.7-24.04 $JOB_NAME $BUILD_NUMBER
bash $BUNDLE_SCRIPTS_DIR/move-to-artefacts.bash $TOOLS_PATH/emboss-24.04 $JOB_NAME $BUILD_NUMBER
