...
bash $BUNDLE_SCRIPTS_DIR/run-in-pod.bash $JOB_NAME $BUILD_NUMBER ubuntu - <<EOF
  ...
  # only used in MEDIPS.R which is commented away
  rm -rf $TOOLS_PATH/R-3.2.3/lib/R/library/BSgenome.*

  # one of the largest files, but most likely unused, because the name is not familiar for tool developers
  rm -rf $TOOLS_PATH/R-3.2.3/lib/R/library/ChemmineDrugs
