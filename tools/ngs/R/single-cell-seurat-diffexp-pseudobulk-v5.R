# TOOL single-cell-seurat-diffexp-pseudobulk-v5.R: "Seurat v5 -Find DE genes between sample groups, pseudobulk" (This tool lists the differentially expressed genes between user defined sample groups for a user defined cluster. This tool can be used for Seurat objects with more than 2 samples in them. Note that )
...
# PARAMETER OPTIONAL test.type: "Which test to use for detecting marker genes" TYPE [negbinom, DESeq2, wilcox, t, poisson] DEFAULT negbinom (Seurat currently implements Wilcoxon rank sum test, bimod \(likelihood-ratio test for single cell gene expression\), roc \(standard AUC classifier\), Students t-test, Tobit-test, poisson, negbinom and DESeq2. The latter three should be used on UMI datasets only, and assume an underlying poisson or negative-binomial distribution.  DESeq2 and negbinom are recommended for pseudobulk analysis. Wilcoxon and t-test require a larger number of samples per group \(≥5\) to have adequate power. Bimod, tobit, and roc are designed for single-cell data and are not appropriate here.)
...
# Note: not assays = "RNA", so that it works also for sct
...
if (nrow(pseudobulk_markers_NAs) != 0) {
...
ggplot(pseudobulk_markers, aes(avg_log2FC, -log10(p_val))) +
  ...
  ggtitle("Volcano plot of pseudobulk results (p_val_adj < 0.01 in red)")
