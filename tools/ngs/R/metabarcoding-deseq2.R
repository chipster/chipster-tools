# TOOL metabarcoding-deseq2.R: "Differential OTU/ASV abundance analysis using DESeq2" (Tests for OTU/ASV differential abundances using DESeq2. A Benjamini-Hochberg multiple inference correction is applied to the data, with OTUs/ASVs filtered using a cut-off value of 0.01. While the analysis can be performed using a phenodata variable comprising >2 sample groups, output files are restricted to comparisons between two groups. Produces a result table and plots OTU differential abundances at a user-specified level of biological organization. Requires a phyloseq object converted into the DESeq2 format as the input, i.e. the file \"deseq2.Rda\" generated by the \"Transform OTU/ASV counts\" tool. Note that the input file should not contain transformed OTU/ASV counts \(raw count data are required\).)
