...
# PARAMETER OPTIONAL only.positive: "Conserved markers: Return only positive marker genes" TYPE [FALSE, TRUE] DEFAULT TRUE (Tool only returns positive markers as default. Change the parameter here if you want to also include the negative markers.)
...
# PARAMETER OPTIONAL pval.cutoff.conserved: "Conserved markers: Adjusted p-value cutoff" TYPE DECIMAL FROM 0 TO 1 DEFAULT 0.05 (Cutoff for the p-value of the conserved cluster marker genes: by default, p-values bigger than 0.05 in any sample are filtered out. This is looking at the max_pval column of the results table.)
...
# PARAMETER OPTIONAL mincellsconserved: "Conserved markers: Minimum number of cells in one of the groups" TYPE INTEGER DEFAULT 3 (How many cells at least there needs to be in a each sample in the cluster in question.)
