...
# PARAMETER OPTIONAL minpct: "Minimum fraction of cells where a gene is expressed" TYPE DECIMAL DEFAULT 0.1 (Test only genes which are detected in at least this fraction of cells in either of the two populations. Meant to speed up testing by leaving out genes that are very infrequently expressed.)
# PARAMETER OPTIONAL threshuse: "Differential expression threshold for a gene" TYPE DECIMAL DEFAULT 0.25 (Limit testing to genes which show at least this fold difference, in natural logarithm scale, between the two groups of cells. Increasing the threshold speeds up testing, but can miss weaker signals.)
...
# 2020-10-11 EK update minpct to v3 default
