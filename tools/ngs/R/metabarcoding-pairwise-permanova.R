# TOOL metabarcoding-pairwise-permanova.R: "Post-hoc pairwise PERMANOVA for OTU/ASV abundance data" (Performs post-hoc pairwise permutational analyses of variance \(PERMANOVA, 999 iterations\), using distance matrices derived from OTU/ASV abundance data. Accepts a single phenodata variable and applies a Benjamini-Hochberg correction to account for multiple testing. Requires an Rda file \(ps_dist.Rda\) produced by the "Distance matrices and ordinations" tool as the input. This tool should only be used after obtaining a statistically significant \(p \< 0.05\) result using the \"Global PERMANOVA for OTU/ASV abundance data\" tool.)
