# TOOL gatk4-createsomaticpanelofnormals.R: "Create Somatic Panel of Normals" (Create a panel of normals (PoN\) containing germline and artifactual sites for use with Mutect2. The tool takes multiple normal sample callsets produced by Mutect2's tumor-only mode and collates sites present in two or more samples into a sites-only VCF. The PoN captures common artifactual and germline variant sites. Mutect2 then uses the PoN to filter variants at the site-level. Tool is based on GATK4 createSomaticPanelOfNormals.)
