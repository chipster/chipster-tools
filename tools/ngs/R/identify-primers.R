...
if (reverse_file == "") {
    ...
    if (adapter5 == "") {
        ...
        matrix <- rbind(
            Forward_reads_5adapter = sapply(orients5, primerHits, fn = forward_file),
    ...
    if (adapter5 == "") {
        matrix <- rbind(
            Forward_reads.3adapter = sapply(orients3, primerHits, fn = forward_file),
            Reverse_reads_3adapter = sapply(orients3, primerHits, fn = reverse_file)
        )
    } else if (adapter3 == "") {
        matrix <- rbind(
            Forward_reads.5adapter = sapply(orients5, primerHits, fn = forward_file),
