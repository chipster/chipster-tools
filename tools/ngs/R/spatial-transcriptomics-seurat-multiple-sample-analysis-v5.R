# TOOL spatial-transcriptomics-seurat-multiple-sample-analysis-v5.R: "Seurat v5 -Combine multiple samples, normalization and PCA" (This tool can be used to combine multiple Seurat objects and perform normalization and PCA for multiple samples at the same time. This tool first normalizes each sample with SCTransform and detects highly variable genes. Then, the normalized samples are combined by merging the samples. After combining the samples, variable genes for the combined Seurat object are selected. Then, PCA is performed on the merged object with the selected variable genes. Finally, if integration is selected, samples are integrated using CCA as the anchor identification method.)
...
# PARAMETER OPTIONAL num.features: "Number of variable genes to return in SCTransform" TYPE INTEGER DEFAULT 3000 (Number of variable genes returned in SCTransform.)
