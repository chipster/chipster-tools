...
# PARAMETER OPTIONAL qtrim: "Should read ends be trimmed based on quality" TYPE [rl: (Trim both ends), f: (Neither end), r: (Right end only), l: (Left end only), w: (Sliding window)] DEFAULT rl (After looking for kmers, remove low quality bases from read ends. Set the quality threshold with the next parameter.)
