...
# PARAMETER OPTIONAL add.colors: "Add custom colors for clusters" TYPE [black, red, blue, green, pink] DEFAULT black (tesktiä)
# PARAMETER OPTIONAL label.size: "Choose size for label text" TYPE INTEGER DEFAULT 5 (Valitse numero)
...
FeaturePlot(object = seurat_obj, features = feature_to_plot, pt.size = point.size, label.size = label.size, label.color = add.colors, label = add.labels, order = as.logical(plotting.order.used))
