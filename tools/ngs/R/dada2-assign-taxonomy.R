...
# PARAMETER species: "Assign sequences to the species level" TYPE [yes, no] DEFAULT no (Do you want to assign the sequences to the species level if there is an exact match 100% identity between ASVs and sequenced reference strains?)
...
if (species == "yes") {
    ...
    # run addSpecies()
    taxa <- addSpecies(taxa, path2, tryRC = tryrc)
}
