# TOOL single-cell-extract-information-v5.R: "Seurat v5 -Extract information from Seurat object" (This tool extracts data from a Seurat object.)
...
# PARAMETER OPTIONAL gene_list: "Print list of the genes" TYPE [yes, no] DEFAULT no (Note that this list contains genes in the active assay of the Seurat object.)
# PARAMETER OPTIONAL var_genes_list: "Print highly variable genes" TYPE [yes, no] DEFAULT no (Note that this list contains variable genes in the active assay of the Seurat object.)
...
if (gene_list == "yes") {
    if (seurat_obj@active.assay == "predictions") {
        stop(paste("CHIPSTER-NOTE: ", "The active assay contains single-cell data integration predictions. The active assay does not contain list of genes. Cannot print list of genes."))
    }
...
# Create a list of the highly variable features in the object
if (var_genes_list == "yes") {
    if (seurat_obj@active.assay == "predictions") {
        stop(paste("CHIPSTER-NOTE: ", "The active assay contains single-cell data integration predictions. The active assay does not contain list of variable genes. Cannot print list of variable genes."))
    } 
    if (length(VariableFeatures(seurat_obj)) == 0) {
        stop(paste("CHIPSTER-NOTE: ", "Variable features have not been set in this seurat object assay. Cannot print list of variable genes."))
    }
