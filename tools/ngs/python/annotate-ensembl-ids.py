# TOOL annotate-ensembl-ids.py: "Annotate Ensembl identifiers" (Annotates Ensembl IDs with gene symbols and descriptions, creates a new table containing these and the values in the original input file. Please note that annotation should be done after other analysis steps. This is because the description column contains sometimes special characters, which can confuse the R/Python code of the analysis tools. The Ensembl IDs can be either the rownames or in the first column of the input table. They can also be in the middle if the name of the column is ensembl_id.)
