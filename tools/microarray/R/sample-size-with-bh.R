...
plot(x, density.estimate.of.p.values,
  type = "l", xlab = "p-value", ylab = "density", col = 1, xlim = c(0, 1),
...
estimate.of.lambda.at.theta <- function(theta, a.n, statistics, n, N, nu) {
  first.integral <- integrate(first.integrand.at.t,
    lower = -1 / a.n, upper = 1 / a.n, theta = theta,
    subdivisions = 100000, abs.tol = 0.00001, statistics = statistics, n = n, a.n = a.n, N = N, nu = nu
  )$value
  second.integral <- integrate(second.integrand.at.t,
    lower = -1 / a.n, upper = 1 / a.n, theta = theta,
    subdivisions = 100000, abs.tol = 0.00001, statistics = statistics, n = n, a.n = a.n, N = N
  )$value
...
pdf(file = "lambda.pdf")
plot(theta.vector, estimate.lambda.of.theta,
  type = "l", col = 3, ylim = c(0, 1.2 * max(estimate.lambda.of.theta)), xlab = quote(theta),
...
pdf(file = "power.pdf")
plot(samplesize, power,
  type = "b", main = paste("Power calculation for FDR = ", round(100 * delta), "%", sep = ""),
