...
if (meth == "RankProd") {
    ...
    if (pairing == "EMPTY") {
        ...
        if (ncol(dat2.1) != ncol(dat2.2)) {
            stop("Paired RankProd error: matrices differ in column number")
        }
...
if (meth == "fast-t-test") {
    if (pairing != "EMPTY") {
        stop("CHIPSTER-NOTE: Fast T-test does not support pairing information")
    }
...
if (meth == "F-test") {
    ...
    if (pairing != "EMPTY") {
        stop("CHIPSTER-NOTE: F-test does not support pairing information")
    }
...
if (meth == "LPE") {
    if (pairing != "EMPTY") {
        stop("CHIPSTER-NOTE: LPE does not support pairing information")
    }
