# TOOL deseq2-transform.R: "Transform read counts" (Transforms read counts for genes for visualization and clustering purposes using the DESeq2 Bioconductor package. You can create the input count table and phenodata file using the tool \"Utilities - Define NGS experiment\". Note that the input file has to contain all the genes, not just differentially expressed genes. If you need to obtain transformed counts for differentially expressed genes, you have to perform the transformation first on the whole count table and then use the interactive Venn diagram to intersect the transformed count table with the list of differentially expressed genes.)
